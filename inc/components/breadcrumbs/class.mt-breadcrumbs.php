...
class Pixova_Lite_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			 'home_prefix'   			=> get_theme_mod('pixova_lite_blog_breadcrumb_menu_prefix', __('You Are Here', 'high-star') ),
			 ...
			 'home_label'      			=> esc_html__( 'Home', 'high-star' ),
			 'tag_archive_prefix'   	=> esc_html__( 'Tag:', 'high-star' ),
			 'search_prefix'        	=> esc_html__( 'Search:', 'high-star' ),
			 'error_prefix' 			=> esc_html__( '404 - Page not Found', 'high-star' ),
		);
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="high-star-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="high-star-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
			}
		}
	...
	private function get_post_type_archive( $linked = TRUE ) {
		...
		if ( $post_type == '' && $page_id ) {
			...
			$archive_title = __( 'Home', 'high-star' );
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<span class="high-star-breadcrumb-sep">%s</span>', $this->separator );
		}
